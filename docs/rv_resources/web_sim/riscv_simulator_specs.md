...
| [`examples/`](examples/) | Every example but DIP to LED — `asm/*.asm`, `c/*.c` — fetched by the page when you select one. Listed in `asm/index.txt` / `c/index.txt` (plain text, not `.md` — static site generators tend to render `.md` rather than serve it verbatim), one markdown-style table row per example; that is the whole menu, no HTML edit needed to add one. |
...
The menu itself is not hardcoded either. `examples/asm/index.txt` and `examples/c/index.txt`
